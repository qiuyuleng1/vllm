...
OMP_NUM_THREADS=24 XFT_PIPELINE_STAGE=4  mpirun \
  -n 1 numactl --all -C 0-25 -m 0 \
    python -m vllm.entrypoints.openai.api_server \
      ...
      --port 8000 \
      --trust-remote-code \
  : -n 1 numactl --all -C 28-53 -m 0 \
    python -m vllm.entrypoints.slave \
      --dtype bf16 \
      --model ${MODEL_PATH} \
      --kv-cache-dtype fp16 \
  : -n 1 numactl --all -C 56-81 -m 1 \
    python -m vllm.entrypoints.slave \
      --dtype bf16 \
      --model ${MODEL_PATH} \
      --kv-cache-dtype fp16 \
  : -n 1 numactl --all -C 84-109 -m 1 \
    python -m vllm.entrypoints.slave \
      --dtype bf16 \
      --model ${MODEL_PATH} \
      --kv-cache-dtype fp16 \
